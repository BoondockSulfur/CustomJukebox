...
- **Long names ran past the edge of a GUI.** The admin panel lists a disc as the real disc item, so its full name became the tooltip and a long one grew past the menu. Inventory titles had the same problem and are worse for it: they are drawn into a fixed-width frame, in bold, and the disc editor pasted in the full disc id — `Edit: music_disc.newbeginnings_theme` ran well past the edge. All four editor titles now budget the fixed prefix and the name together, and the disc editor drops the namespace prefix its ids share — `Edit: music_disc.newbeginnings_theme` becomes `Edit: newbeginnings_theme`. The prefix is read from the server's own ids rather than assumed, so a pack using `custom.` or anything else is handled too; where ids do not all share one, nothing is stripped. List entries and titles now shorten a name that does not fit — the item itself is untouched and keeps its full name. Length is measured by *visible* characters, since markup is no guide to width: a gradient can be forty characters of tags around two visible ones, and cutting the raw string would slice a tag in half. Playlist and category entries and the preview lines in tooltips are capped the same way.
...
### Added
- **A disc and an ambient zone no longer play over each other.** The two systems did not know about one another, so a player standing in a zone who put a disc on heard both at once. A disc is deliberate and a zone is background, so the zone now falls silent for the listeners who can hear the disc and returns when it ends — not between the tracks of a playlist, where the next one starts immediately. `ambient-zones.pause-during-disc` turns it off for anyone who wants them layered.
