...
- **Long names ran past the edge of a GUI.** The admin panel lists a disc as the real disc item, so its full name became the tooltip and a long one grew past the menu; inventory titles, drawn into a fixed-width frame, had the same problem. List entries and titles now shorten a name that does not fit — the item itself is untouched and keeps its full name. Length is measured by *visible* characters, since markup is no guide to width: a gradient can be forty characters of tags around two visible ones, and cutting the raw string would slice a tag in half. Playlist and category entries and the preview lines in tooltips are capped the same way.
