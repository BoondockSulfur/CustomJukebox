...
### Added
- **Volume in decibels**, for the server, zone and personal commands alike: `-6db` is half, `-12db` a quarter, `-20db` a tenth. The linear scale is hard to judge by ear — 0.5 does not sound half as loud — while a decibel step always sounds like the same step.
- **Presets and percentages everywhere.** `/cjb zone volume` and `/cjb music volume` take the same words as the server command (`silent`, `quiet`, `normal`, `loud`, `max`) and values such as `30%`, instead of bare numbers only.
- **Volume feedback names what it did**: `0.30 (30%, -10.5 dB)`. Above 1.0 it also says the sound is already at full loudness and how far it now carries — Minecraft widens the audible radius (`volume * 16` blocks) rather than raising the gain, so comparing 4.0 with 1.0 next to a jukebox sounds identical and the command looks broken.
- **`/cjb zone volume <zone> <value> norestart`** for anyone who would rather the current track finish at its old volume.
- **`ambient-zones.pause-during-disc`** to keep discs and zone music layered, if that is what you want.

### Changed
- All three volume commands share one parser, so presets, percentages and ranges cannot drift apart between them.
- The author prompt in the disc editor shows the formatting example itself instead of pointing at the display name prompt — whoever opens that field first never saw the example.
- Editor titles drop the namespace prefix a server's disc ids share: `Edit: music_disc.newbeginnings_theme` becomes `Edit: newbeginnings_theme`. The prefix is read from the ids in use rather than assumed, so `custom.` or a multi-level one works the same way; where ids do not all share a prefix, nothing is stripped.

### Fixed
- **Colour codes in a disc's author field were printed instead of applied.** `&eArtist` showed up in the tooltip as the literal text `<yellow>Artist`. The author line is assembled as `§7By: §e` plus the author, and MiniMessage rejects a section sign outright — so every such call threw, fell back to the legacy serializer, and that serializer printed the already-converted tags as visible text. Unformatted names came through the fallback looking correct, which is why this went unnoticed; the display name, which carries no such prefix, always worked, making formatting look supported everywhere except the one field where it silently was not.

  Legacy codes are now recognised with either marker (`&` or `§`) and in either case, so `&c`, `§c` and `&C` all apply, hex (`&#ff8800`) and gradients survive, and the same fix covers every other place that mixed the two markers — fragment lore, the jukebox subtitle, GUI text.
- **A disc and an ambient zone played over each other.** The two systems had no knowledge of one another, so a player standing in a zone who put a disc on heard two songs at once. A disc is deliberate and a zone is background, so the zone now falls silent for the listeners who can hear the disc and returns when it ends — not between the tracks of a playlist, where the next one starts immediately and the zone would blip in for a moment.
- **A zone's own volume followed the server volume for anyone with a personal volume set.** The per-listener calculation divided the zone's volume by the server volume, so a zone deliberately set to a fixed level swung with a setting it had opted out of: at the shipped server volume of 4.0 a zone at 0.2 collapsed to a twentieth of the player's setting, and turning the server volume down made that zone jump up. A zone with its own volume is absolute now; only zones left on `inherit` follow the server, and a personal volume applies on top as a factor where 1.0 means "as configured".
- **`/cjb music volume` did nothing until the current track ended.** It stored the value and said so, which on a looping ambient zone means minutes of no audible change — while `/cjb music on|off` beside it takes effect at once. Setting a personal volume now restarts what that one player is hearing, leaving everyone else alone.
- **`/cjb volume ... restart` ignored ambient zones.** It restarted jukebox playbacks only, so a zone set to `inherit` kept its old volume until its current track ended and the flag looked broken wherever ambient music was running. The same applied to `/cjb mute` and `/cjb unmute`. Zones on `inherit` are restarted as well now.
...
- **Tab completion never offered a quiet value.** `/cjb zone volume <zone>` suggested `inherit, 1, 2, 3, 4` only, and since 1.0 is already full loudness, "turn it down" reliably ended at 1 and stayed loud. The suggestions start at the quiet end now: presets, `0.05` through `1`, and common decibel values.
- **`/cjb volume` looked broken to an admin who had set a personal volume.** A personal volume replaces the server volume rather than scaling it, so the admin's own command changed nothing they could hear. The command now says which value is winning and names the reset that undoes it.
- **Chat confirmations printed the markup they had just applied.** Setting a name or author replied with the raw `<gradient:#ff0000:#0000ff>Name</gradient>` while the item itself came out correctly coloured, because the confirmation went through a serializer that only understands `&` codes. Values a user wrote are rendered the way the game renders them now.
- **Long names ran past the edge of the GUI.** The admin panel lists a disc as the real disc item, so a long display name became a tooltip wider than the menu; the entry shown in the list is shortened now, while the item players receive keeps its full name. Playlist and category entries and the preview lines in tooltips are shortened the same way.
- **Editor titles ran past the frame.** Inventory titles are drawn into a fixed width, in bold, and the disc editor pasted in the full disc id. All five editor titles now budget their fixed prefix and the variable part together. Length is measured in *visible* characters, since markup is no guide to width: a gradient can be forty characters of tags around two visible ones, and cutting the raw string would slice a tag in half — so a name that has to be shortened loses its formatting rather than its closing tags.
- **Language files on disk fell behind and never caught up.** A shipped message is only written out when the file does not exist yet, so a server that has run since an early version keeps its original file forever — one long-running server was missing 170 of its 226 messages. Nothing was visibly broken, since missing keys resolve through the bundled defaults, but an admin cannot reword or translate a message that is not in the file. Messages the shipped file has and the server's copy does not are written in on every start, strictly additively: an existing line is never touched, so rewordings and translations survive, and comments are preserved.
